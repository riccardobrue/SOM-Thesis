...
class SOM:
    ...
    def __init__(self, input_shape, map_size_n, num_expected_iterations, session):
        ...
        self.sigma_act = tf.constant(2.0 * (reduce(lambda x, y: x * y, self.input_shape, 1) * 0.05) ** 2,
                                     dtype=tf.float32)
    ...
    def initialize_graph(self):
        self.weights = tf.Variable(tf.random_uniform((self.n * self.n,) + self.input_shape, 0.0,
                                                     1.0))  ##TODO: match with input type, and check that my DQN implementation actually uses values in 0-255 vs 0-1?
    ...
    def train(self, input_x):  # TODO: try training a batch all together, to optimize gpu usage?
        ...
        bmu_index, diff = self.session.run([self.bmu_index, self.diff], {self.input_placeholder: [input_x],
                                                                         self.current_iteration: self.num_iterations})

        # Update the network's weights
        self.session.run(self.update_weights, {self.diff_2: diff, self.dist_sliced: self.dist[bmu_index[0], :],
                                               self.current_iteration: self.num_iterations})
    ...
    def get_batch_winner(self, batch_input):
        ...
        indices = self.session.run([self.bmu_index],
                                   {self.input_placeholder: batch_input, self.current_iteration: self.num_iterations})
