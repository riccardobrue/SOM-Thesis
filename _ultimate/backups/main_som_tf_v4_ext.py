...
for prot_index in range(0, len(cols)):
    ...
    # SAVE THE CHART
    plt.savefig(directory_path + 'HND_' + label_names[prot_index] + '.png')
...
for prot_index in range(0, len(cols)):
    ...
    # SAVE THE CHART
    plt.savefig(directory_path + 'FND_' + label_names[prot_index] + '.png')
...
for prot_index in range(0, len(cols)):  # iterate each protocol
    ...
    for i, u in enumerate(unique_classes):
        my_dpi = 96
        plt.figure(_figure_counter, figsize=(800 / my_dpi, 800 / my_dpi), dpi=my_dpi)
        _figure_counter = _figure_counter + 1
