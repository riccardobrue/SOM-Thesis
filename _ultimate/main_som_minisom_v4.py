...
def execute(clustering_cols, clustering_data_type="all"):
    ...
    folder_prefix_2 = "tmp-1000_"  # to select the restored checkpoint
    ...
    manually_picked_som_dim = 40  # if heuristic_size is False, this will be the chosen som's side size
...
if __name__ == "__main__":
    ...
    sim_results_continue = [6, 7, 8, 9, 10, 11, 12, 13]  # for "all" continuing from net_attributes

    sim_h_f_nd_results = [0, 1, 2, 3]  # ['REECHD H/FND' 'HEED H/FND' 'ERHEED H/FND' 'FMUC H/FND']

    # change this!
    clust_type = "all"  # clustering type: [all, net, net+hnd, net+fnd, sim, hnd, fnd, all-net, all-sim, all-hnd, all-fnd]
    ...
    for nt_index in net_attributes:
        for sim_index in sim_results_continue:
            print("---------> ",nt_index, " - ", sim_index)
            execute(clustering_cols=[nt_index, sim_index], clustering_data_type="all")
